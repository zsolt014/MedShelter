...
function openResidentModal(id){
  ...
  if(id){
    ...
    document.getElementById('res_birthDate').value = toIsoDate(r.birthDate);
    ...
    document.getElementById('res_buletinNumber').value = r.buletinNumber || '';
    document.getElementById('res_buletinExpiry').value = toIsoDate(r.buletinExpiry);
...
if(residentForm){
  residentForm.addEventListener('submit', function(e){
    ...
    const birthDate = toDotDate(document.getElementById('res_birthDate').value);
    ...
    const buletinNumber = document.getElementById('res_buletinNumber').value.trim();
    const buletinExpiry = toDotDate(document.getElementById('res_buletinExpiry').value);
    ...
    if(editId){
      ...
      if(r){
        Object.assign(r, {
          name, room, status, doctor, birthDate, taj,
          phone: phone || '—', notes, buletinNumber, buletinExpiry
        });
      ...
      residents.push({
        id: nextResidentId++, name, room, status, doctor, birthDate, taj,
        phone: phone || '—', notes, buletinNumber, buletinExpiry,
        medications: [], letters: []
      });
...
function openResidentDetail(id){
    ...
    <tr>
      ...
      <td>
        <div class="row-actions" style="justify-content:flex-start;">
          <button class="icon-btn" title="Megújítás (3 hónapra)" onclick="recordPrescription(${r.id}, ${m.id})">
  ...
  `).join('') : `<tr><td colspan="6"><div class="empty-state">Ehhez a lakóhoz még nincs felvéve gyógyszer.</div></td></tr>`;
  ...
  let buletinValidityHtml = '<span class="validity-tag" style="color:var(--text-muted);">Nincs megadva</span>';
  if(r.buletinExpiry){
    const urg = urgencyOf(r.buletinExpiry);
    const label = urg === 'urgent' ? 'Lejárt' : urg === 'soon' ? 'Hamarosan lejár' : 'Érvényes';
    const dotClass = urg === 'urgent' ? 'urgent' : urg === 'soon' ? 'soon' : 'today';
    buletinValidityHtml = `<span class="validity-tag"><span class="dot ${dotClass}"></span>${label} (${r.buletinExpiry})</span>`;
  }
  ...
  detail.innerHTML = `
    ...
    <div class="profile-grid">
      <div class="profile-card">
        ...
        <div class="profile-sub">${escapeHtml(r.room)}. szoba · ${escapeHtml(r.status)}</div>
        ...
        <div class="profile-field">
          <div class="profile-field-label">Buletin szám</div>
          <div class="profile-field-value">${r.buletinNumber ? escapeHtml(r.buletinNumber) : '—'}</div>
        </div>
        <div class="profile-field">
          <div class="profile-field-label">Buletin érvényessége</div>
          <div class="profile-field-value">${buletinValidityHtml}</div>
        </div>
        <div class="profile-field">
      ...
      <div>
        <div class="subpanel">
          <div class="subpanel-header">
            <h3>Gyógyszerek</h3>
            <button class="btn btn-secondary btn-sm" onclick="openMedicationModal(${r.id}, null)">+ Gyógyszer felírása</button>
          </div>
          <table>
            <thead><tr><th>Gyógyszer</th><th>Adagolás</th><th>Havi menny.</th><th>Utolsó iratás</th><th>Következő</th><th>Műveletek</th></tr></thead>
...
function openLetterModal(residentId, letterId){
  clearLetterErrors();
  if(letterForm) letterForm.reset();
  ...
  if(letterId){
    ...
    document.getElementById('letter_valid').value = toIsoDate(l.valid);
...
if(letterForm){
  letterForm.addEventListener('submit', function(e){
    ...
    const validDate = toDotDate(document.getElementById('letter_valid').value);
